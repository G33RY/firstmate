...
[ "$TOOLS_SET" -eq 0 ] || [ -n "$TOOLS" ] || {
  echo "error: --tools requires a non-empty value; pass 'none' for the conservative default explicitly" >&2
  exit 1
}
...
launch_template() {
  ...
  claude)
    ...
    printf '%s' '__TOOLSFLAG____MODELFLAG____EFFORTFLAG__"$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
    ;;
