...
On that path teardown hands the completion links it would have closed with to `bin/fm-captain-hold.sh retain <task-id>`, which records one `Deliverable of the finished work: ...` line in the task body (idempotent per line, existing body preserved) and runs `tasks-axi reopen` so the row returns to Queued while keeping its hold.
