...
After endpoint and worktree cleanup succeeds, teardown hands the completion links it would have closed with to `bin/fm-captain-hold.sh retain <task-id>`, which records one `Deliverable of the finished work: ...` line in the task body (idempotent per line, existing body preserved) and runs `tasks-axi reopen` so the row returns to Queued while keeping its hold.
`answer` and `retain` serialize their complete body-and-state transactions on the same per-task metadata lock, with teardown retaining ownership through its final record removal so a concurrent answer cannot be overwritten or reopened.
...
No `state/<id>.backlog-close` record is staged on this path: replaying one at the next session start would close the captain call, which is the loss the deferral exists to prevent.
