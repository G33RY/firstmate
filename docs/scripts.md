...
| `fm-tool-scope-lib.sh`   | Single owner of the `--tools` spec syntax; generates a spawn's scoped MCP-config file and chrome flags |
...
| `fm-pr-describe.sh`      | Rewrite a task's GitHub PR body into the captain's human-reviewable format, archiving the prior body first |
