...
The judge's own brief says it will be re-invoked; `bin/fm-babysitter-invoke-lib.sh` is what actually does that, deterministically, never the judge's own responsibility - the same reasoning as the liveness layer below. `state/babysitter.check.sh` calls it on every watcher poll (`bin/fm-watch.sh`'s `CHECK_INTERVAL`, default 300s), right after the liveness check; it is not a second scheduler beside that check, just a second thing the same poll does.

It triggers a pass when either is true: unread ledger content is waiting (`bin/fm-babysitter-ledger.sh unread`), or `FM_BABYSITTER_SWEEP_INTERVAL_SECS` (default `1800`) has elapsed since the last dispatched pass, so a long quiet stretch still gets one stall sweep even with no new dialog to judge (a stall is read from the fleet snapshot, not the ledger). It never dispatches to a target `fm_backend_agent_state` does not report `alive` (a dead or just-relaunched judge is the liveness layer's problem to fix first).

It never overlaps a pass: delivery is one ordinary steer through `bin/fm-send.sh`'s durable inbox (never typed at the pane directly), and `fm-task-inbox-lib.sh`'s own contract is that the worker's `mv` into `handled/` IS the acknowledgement - so one unhandled record in the judge's steering inbox means a pass is already dispatched and not yet picked up, and the next poll skips. A record older than `FM_BABYSITTER_INVOKE_STUCK_SECS` (default `600`) is instead read as orphaned - the judge died mid-pass and the liveness layer relaunched it into a fresh session that never saw the old doorbell - and is retired into `handled/` by the check itself so the busy gate cannot wedge open forever.
...
`bin/fm-wake-drain.sh`'s `PARKED AT CHECKPOINT` section and `bin/fm-classify-lib.sh`'s `status_is_parked_checkpoint` detect a `mode=no-mistakes` ship task whose latest status line is a bare `done:` (no PR URL) - the deliberate handoff point where the worker has committed and is waiting for firstmate to trigger `/no-mistakes` (`AGENTS.md` section 7). The status line only proves the past, since a worker never has to append another line once validation starts, so a flagged candidate is confirmed against live state (`crew_is_provably_working`, `bin/fm-classify-lib.sh`) before it is reported; an unreadable or ambiguous live read does not clear it, since a missed real stall outweighs one redundant line. This is not a worker malfunction; it is firstmate's own unmet obligation, and it needs no LLM judgment to detect. It queues its own `check:` wake (idempotent per task) and, past `FM_BABYSITTER_PARKED_TIER2_SECS` (default 1800s), fires the tier-2 nudge directly through `bin/fm-babysitter-ntfy.sh`.
