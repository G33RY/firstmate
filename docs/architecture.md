...
A stale pane whose only liveness evidence is a currently active no-mistakes run step (rather than a merely busy pane) is exempt from `FM_STALE_ESCALATE_SECS` entirely: it takes the same bounded `FM_PAUSE_RESURFACE_SECS` cadence as a declared wait, since a legitimate wait on that step (a held PR merge, for one) can outlast the short wedge timer by a wide margin, and only escalates once its own reported position stops advancing across successive bounded rechecks - distinguishing a step that is merely quiet from one that is not moving.
...
A `https://github.com/<owner>/<repo>/pull/<n>` URL invokes `gh-axi pr merge <n> --repo <owner>/<repo>`, defaults to `--squash`, and preserves explicit merge-method flags; an explicit, opt-in `--admin-bypass` instead invokes the real `gh` CLI with `--admin` after a live `enforce_admins` check, since gh-axi's merge subcommand exposes no such flag — `bin/fm-pr-merge.sh`'s header owns that flag's mechanics.
