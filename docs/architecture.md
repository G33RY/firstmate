...
`--auto`, `--admin`, and branch-deletion flags are refused unless `--attended-override` is passed for an explicit captain instruction; that override never skips the live green check, the away-grant check, or a captain hold - this is also the sanctioned path for an authorized admin-bypass merge (`--attended-override -- --admin`), replacing the fork's former dedicated `--admin-bypass` flag with the same capability under the same explicit-instruction gate.
