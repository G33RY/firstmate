...
Holding the work item the question gates is safe for exactly that reason: cleanup defers its close while the row is still an open captain call, records the finished work's deliverable on it, and returns it to the queue it waits in, so the call keeps reading as the captain's own with nothing running behind it.
