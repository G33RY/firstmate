...
- A message **without** the current operational prefix or a legacy bare marker, and **not** starting with `/afk` -> the captain is back.
  ...
  Once the check exits successfully, load `decision-ledger` and file any captain-level call, worker overrule, or firstmate mistake made autonomously while away that never got a ledger entry, before resuming ordinary captain work.
